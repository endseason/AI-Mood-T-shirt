...
const TODAYTOP_ITEMS = [
  {
    kind: 'image' as const,
    title: '今日爆梗🔥',
    imageUrl: '/todaytop/新对话 (2).png',
  },
   {
    kind: 'image' as const,
    title: '今日灵感 02',
    imageUrl: '/todaytop/截屏2026-04-11 01.27.11.jpeg',
  },
  {
    kind: 'image' as const,
    title: '今日热梗 01',
    imageUrl: '/todaytop/1775842106e94c.png',
  },
  {
    kind: 'image' as const,
    title: '今日灵感 03',
    imageUrl: '/todaytop/截屏2026-04-11 01.27.42.png',
  },
  {
    kind: 'image' as const,
    title: '今日热梗 02',
    imageUrl: '/todaytop/17758418288b4e.png',
  },
  {
    kind: 'image' as const,
    title: '今日灵感 04',
    imageUrl: '/todaytop/截屏2026-04-11 01.26.14.png',
  },
  {
    kind: 'image' as const,
    title: '今日灵感 05',
    imageUrl: '/todaytop/96efe0a5-3cca-4faf-97ff-b1d65e6a08ef.jpeg',
  },
  {
    kind: 'image' as const,
    title: '今日热梗 03',
    imageUrl: '/todaytop/17758414282108.png',
  },
];
...
const InspirationFlow: React.FC<Props> = ({ onSelect }) => {
  ...
  const showTodayTop = activeFilter === '全部' || activeFilter === '今日爆梗';
  const showOnlyTodayTop = activeFilter === '今日爆梗';
        ...
        <div className="grid grid-cols-2 gap-5">
          {TODAYTOP_ITEMS.map((item) => (
            <div
              ...
              className="rounded-2xl overflow-hidden border border-black/10 bg-transparent shadow-none"
            >
              {item.kind === 'image' ? (
                <button
                  type="button"
                ...
                >
                  ...
                  <div className="px-4 py-3 border-t border-black/5 bg-transparent">
                    <div className="text-base font-semibold text-zinc-800 leading-tight tracking-wide">{item.title}</div>
                  </div>
                </button>
              ) : (
                <div className="min-h-[220px] px-5 py-5 flex flex-col justify-between bg-transparent">
                  <div className="text-xs font-mono text-zinc-500 tracking-[0.24em] uppercase">{item.title}</div>
                  <div className="text-[30px] leading-[1.05] font-black text-zinc-900 tracking-tight max-w-[10ch]">
                    {item.text}
                  </div>
                </div>
              )}
